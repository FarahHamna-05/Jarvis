...
function JobRow({
    ...
    >
      <div className="min-w-0 flex-1 space-y-1">
        <div className="flex flex-wrap items-center gap-2">
          <h3 className="text-sm font-semibold text-slate-900 group-hover:text-slate-700 transition-colors sm:text-base">
            {job.title}
          </h3>
          {job.featured && (
        ...
        <p className="text-xs text-slate-500 sm:text-sm line-clamp-1">
          {job.description}
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap sm:shrink-0">
        ...
        <span className="ml-auto flex size-7 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition-colors duration-300 group-hover:bg-slate-900 group-hover:text-white sm:ml-3">
  ...
  if (renderLink) {
    return (
      <li className="border-b border-slate-100 last:border-b-0 list-none">
...
function CtaBanner({
  ...
  const linkContent = (
    <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-900 hover:underline underline-offset-4 transition-all">
      {cta.linkLabel}
  ...
  return (
    <div className="mt-6 flex flex-col items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/80 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3">
        <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-white border border-slate-200 text-slate-600">
          <FiBriefcase className="size-3.5" />
        </span>
        <p className="text-sm text-slate-600">{cta.text}</p>
      </div>
      {renderedLink}
    </div>
  );
...
export default function Career1({
  ...
  return (
    <section className={cn('w-full bg-white py-8', className)}>
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 space-y-4 sm:mb-12">
          {badge && (
            <div className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
              <HiSparkles className="size-3 text-slate-900" />
              {badge}
            </div>
          )}
            ...
            <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl lg:text-5xl">
              ...
              {headingHighlight && (
                <span className="relative inline-block">
                  <span className="relative z-10 text-slate-900">{headingHighlight}</span>
                  <span
                    aria-hidden="true"
                    className="absolute inset-x-0 bottom-0.5 h-[6px] rounded-full bg-red-500/20"
                  />
                </span>
              )}
            </h2>
        ...
        <div className="rounded-xl border border-slate-200 bg-white px-4 sm:px-6 shadow-sm">
          {filteredJobs.length > 0 ? (
            ...
            <div className="flex flex-col items-center gap-3 py-14 text-center">
              ...
              <p className="text-sm font-medium text-slate-900">No openings right now</p>
